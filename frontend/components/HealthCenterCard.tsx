...
export default function HealthCenterCard({ center, distance }: HealthCenterCardProps) {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-4 card-hover shadow-sm">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1 mt-1">
            <span className="text-xs bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full">{center.wilaya}</span>
          </div>
          {center.address && (
      ...
      {center.phone && (
        <a
          href={`tel:${center.phone}`}
          className="mt-3 w-full flex items-center justify-center gap-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 py-2 rounded-xl text-sm font-medium transition-colors"
        >
